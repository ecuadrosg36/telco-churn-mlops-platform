...
class ChurnPredictionRequest(BaseModel):
    ...
    senior_citizen: Literal[0, 1] = Field(..., alias="SeniorCitizen", description="Whether customer is senior (0=No, 1=Yes)")
    partner: Literal['Yes', 'No'] = Field(..., alias="Partner", description="Whether customer has a partner")
    dependents: Literal['Yes', 'No'] = Field(..., alias="Dependents", description="Whether customer has dependents")
    
    # Account information
    tenure: conint(ge=0, le=100) = Field(..., description="Number of months as customer (0-100)")
    
    # Service subscriptions
    phone_service: Literal['Yes', 'No'] = Field(..., alias="PhoneService", description="Phone service subscription")
    ...
    # Charges
    monthly_charges: confloat(ge=0.0) = Field(..., alias="MonthlyCharges", description="Monthly charge amount")
    total_charges: confloat(ge=0.0) = Field(..., alias="TotalCharges", description="Total charges to date")
...
class ChurnPredictionResponse(BaseModel):
    ...
    prediction: Literal['Yes', 'No'] = Field(..., description="Churn prediction (Yes/No)")
    churn_probability: float = Field(..., ge=0.0, le=1.0, description="Probability of churn (0-1)")
    risk_tier: Literal['low', 'medium', 'high'] = Field(..., description="Risk tier based on probability")

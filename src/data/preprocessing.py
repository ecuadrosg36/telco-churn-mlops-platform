    ...
    def transform(self, X: pd.DataFrame) -> pd.DataFrame:
        ...
        if "TotalCharges" in X.columns:
            ...
            if missing_total_charges > 0:
                logger.info(
                    f"Filling {missing_total_charges} missing TotalCharges with 0"
                )
    ...
    def transform_target(self, y: pd.Series) -> np.ndarray:
        ...
        if self.target in self.encoders:
            ...
            y_str = y_str.map(
                lambda x: (
                    x
                    if x in self.encoders[self.target].classes_
...
class FeatureScaler(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X, y=None):
        ...
        if self.feature_names:
            ...
            logger.info(
                f"Fitted scaler for {len(self.feature_names)} numerical features"
            )
...
def preprocess_data(
    df: pd.DataFrame, target_col: str = None, fitted_pipeline: Pipeline = None
    ...
    if y is not None:
        ...
        y_processed = pd.Series(
            encoder.transform_target(y), index=y.index, name=target_col
        )
    else:
        y_processed = None

...
def generate_predictions(
    ...
    if model_version:
        ...
        try:
            ...
            if versions:
                metadata = registry.get_model_metadata(version=int(versions[0]['version']))
            else:
                metadata = {}
    
    logger.info(f"Using model: v{metadata.get('version', 'unknown')} ({metadata.get('stage', 'unknown')})")
    ...
    logger.info(f"  Predicted churn rate: {(predictions == 1).sum() / len(predictions) * 100:.2f}%")
...
def save_predictions(
    ...
    logger.info(f"  High risk customers: {(predictions_df['risk_tier'] == 'high').sum():,}")
    logger.info(f"  Medium risk customers: {(predictions_df['risk_tier'] == 'medium').sum():,}")
    logger.info(f"  Low risk customers: {(predictions_df['risk_tier'] == 'low').sum():,}")
...
def batch_predict(
    ...
    try:
        ...
        # Save predictions
        output_file = save_predictions(df, predictions, probabilities, metadata, output_path)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Run batch predictions for Telco Churn')
    parser.add_argument(
        '--data-path',
        type=str,

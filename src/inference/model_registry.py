...
class ModelRegistry:
    ...
    def get_model_metadata(
        self, stage: Optional[str] = None, version: Optional[int] = None
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            logger.info(
                f"Retrieved metadata for {self.model_name} v{metadata['version']} ({metadata['stage']})"
            )
            return metadata
    ...
    def promote_model(
        self, version: int, to_stage: str, archive_existing: bool = True
    ) -> None:
        """
        ...
        try:
            ...
            if archive_existing and to_stage in ["Staging", "Production"]:
                existing_versions = self.client.get_latest_versions(
                    self.model_name, stages=[to_stage]
                )

                for existing in existing_versions:
                    logger.info(
                        f"Archiving existing {to_stage} model v{existing.version}"
                    )
                    self.client.transition_model_version_stage(
                        name=self.model_name, version=existing.version, stage="Archived"
                    )

            # Promote new version
            self.client.transition_model_version_stage(
                name=self.model_name, version=version, stage=to_stage
            )

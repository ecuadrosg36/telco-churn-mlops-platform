...
class DriftDetector:
    ...
    def __init__(self):
        ...
        self.drift_share_threshold = self.config.get(
            "monitoring.drift_thresholds.dataset_drift_share", 0.3
        )
        self.stattest_threshold = self.config.get(
            "monitoring.drift_thresholds.stattest_threshold", 0.05
        )
    ...
    def generate_target_drift_report(
        ...
        if (
            self.target not in reference_data.columns
            or self.target not in current_data.columns
        ):
            logger.warning(
                f"Target column '{self.target}' not found in data, skipping target drift report"
            )
            return None, None
    ...
    def generate_all_reports(
        self, current_data_path: Path, reference_data_path: Optional[Path] = None
    ) -> dict:
        """
        ...
        # Generate data quality report
        _, quality_path = self.generate_data_quality_report(
        ...
        target_report, target_path = self.generate_target_drift_report(
            reference_data, current_data
        )
        if target_path:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--reference-data", type=str, help="Path to reference data CSV (optional)"
    )

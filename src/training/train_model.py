...
def load_and_validate_data() -> pd.DataFrame:
    ...
    if validation_result.warnings:
        logger.warning(
            f"Data validation completed with {len(validation_result.warnings)} warnings"
        )
    else:
...
def evaluate_model(model, X_train, y_train, X_val, y_val, model_name: str) -> dict:
    ...
    logger.info(
        f"  Training   - Accuracy: {metrics['train_accuracy']:.4f}, F1: {metrics['train_f1']:.4f}, AUC: {metrics['train_auc']:.4f}"
    )
    logger.info(
        f"  Validation - Accuracy: {metrics['val_accuracy']:.4f}, F1: {metrics['val_f1']:.4f}, AUC: {metrics['val_auc']:.4f}"
    )
...
def main(model_type: str = "both"):
    ...
    try:
        ...
        (
            ...
            y_test_prep,
            pipeline,
        ) = prepare_features(X_train, X_val, X_test, y_train, y_val, y_test)
        ...
        if model_type in ["advanced", "both"]:
            ...
            run_id = log_to_mlflow(
                advanced_model, pipeline, advanced_metrics, advanced_params, "XGBoost"
            )

...
def mock_loaded_model(mock_sklearn_model, mock_preprocessing_pipeline, monkeypatch):
    ...
    monkeypatch.setattr(model_loader, 'preprocessing_pipeline', mock_preprocessing_pipeline)
    monkeypatch.setattr(model_loader, 'model_metadata', {
        'version': '1',
        'stage': 'Production',
        'name': 'test-model'
    })
...
class TestPredictionEndpoint:
    ...
    def test_predict_with_valid_data(self, client, sample_churn_request, mock_loaded_model):
...
class TestBatchPredictionEndpoint:
    ...
    def test_batch_predict_enforces_max_size(self, client, sample_churn_request):
        ...
        large_batch = {
            'customers': [sample_churn_request] * 1001
        }
